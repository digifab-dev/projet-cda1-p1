...
function getBasket() {
    ...
    for (let i = 0; i < data.length; i++) {
        if (data[i]) {
            basket.push([catalog[i][0], catalog[i][1].toFixed(2) + ' €', data[i], Number(catalog[i][1] * data[i]).toFixed(2) + ' €']);
        }
    }
    return basket;
}
...
function displayBasket() {
    ...
    let orderShipping = document.getElementById('order-shipping').firstElementChild;
...
function displayInfosBasket() {
    ...
    if (infosPrice && infosTotal) {
        ...
        infosTotal.textContent = (localStorage.getItem('TOTAL_ITEMS') || 0) + ' article(s)';
    }
}
...
if (deleteBasketId) {
    deleteBasketId.addEventListener('click', function () {
        let result = confirm('voulez-vous vraiment supprimer votre panier ? Cliquez sur "OK" pour supprimer. Sinon cliquez sur "annuler"');
        if (result) {
            deleteBasket();
            let basket = document.getElementById('basket');
            let basketEmpty = document.getElementById('basket-empty');
